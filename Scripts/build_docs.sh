...
TARGETS="${TARGETS:-MLXRFDETR}"
HOSTING_BASE_PATH="${HOSTING_BASE_PATH:-mlx-swift-rf-detr}"
REPO_URL="${REPO_URL:-https://github.com/mnmly/mlx-swift-rf-detr}"
...
if [[ -z "${TOOLCHAINS:-}" && ( "${EMIT_MARKDOWN:-0}" == "1" || "${EMIT_LLMS_TXT:-0}" == "1" ) \
    && -f "$HOME/.swiftly/env.sh" ]]; then
    # Only reach for a swift.org toolchain when the experimental Markdown export is requested;
    # a plain docs build should use the active (Xcode) toolchain so it matches the SDK.
